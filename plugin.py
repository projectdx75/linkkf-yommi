...
plugin_info = {
    ...
    'home' : 'https://github.com/taeminlee/linkkf',
    'more' : '',
}
#########################################################
...
def detail(sub): 
    if sub == 'setting':
        ...
        setting_list = db.session.query(ModelSetting).all()
        arg = Util.db_list_to_dict(setting_list)
        arg['current_code'] = LogicLinkkf.current_data['code'] if LogicLinkkf.current_data is not None else None
        return render_template('%s_%s.html' % (package_name, sub), arg=arg)
    elif sub == 'log':
...
def ajax(sub):
    ...
    # 요청
        ...
        try:
            ...
            ret = LogicLinkkf.get_title_info(code)
            return jsonify(ret)
        except Exception as e: 
            logger.error('Exception:%s', e)
            logger.error(traceback.format_exc())
        ...
        try:
            ...
            count = 0
            for c in code_list:
                info = LogicLinkkf.get_info_by_code(c)
                if info is not None:
                    tmp = LogicQueue.add_queue(info)
                    count += 1
            ret['ret'] = 'success'
